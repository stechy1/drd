...
final class FirebaseConvertors {
    ...
    private static final FirebaseConvertor USER_CONVERTOR = snapshot -> {
        final Map<String, Object> map = new HashMap<>();
        map.put(IAuthService.COLUMN_ID, snapshot.getKey());
        map.put(IAuthService.COLUMN_NAME, snapshot.child(IAuthService.COLUMN_NAME).getValue(String.class));
        map.put(IAuthService.COLUMN_PASSWORD, snapshot.child(IAuthService.COLUMN_PASSWORD).getValue(String.class));
        return map;
    };
    ...
    static {
        ...
        CONVERTORS.put(IAuthService.FIREBASE_CHILD, USER_CONVERTOR);
