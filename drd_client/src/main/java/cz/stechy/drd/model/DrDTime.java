...
public final class DrDTime implements Comparable<DrDTime> {
     ...
     */
    private void changeInternal(Number oldValue, Number newValue, Callback subtracter,
        Callback adder) {
        synchronized (lock) {
            if (changing) {
                return;
     ...
     * @return {@link DrDTime}
     */
    private DrDTime addInternal(int value, int modifier, int multiplier, Callback callback,
        Getter getter) {
        if (value < 0) {
            return this;
        }
     ...
     * @return {@link DrDTime}
     */
    private DrDTime subtractInternal(int value, int modifier, int multiplier, Callback callback,
        Getter getter) {
        if (value < 0) {
            return this;
        }
    ...
    public DrDTime addYear(int year) {
        return addInternal(year, MAX_YEAR, MULTIPLIER_YEAR, value -> {
        }, this::getYear);
    }

    public DrDTime subtractYear(int year) {
        return subtractInternal(year, MAX_YEAR, MULTIPLIER_YEAR, value -> {
        }, this::getYear);
    }
    ...
    public DrDTime subtractMonth(int month) {
        return subtractInternal(month, MAX_MONTH, MULTIPLIER_MONTH, this::subtractYear,
            this::getMonth);
    }
    ...
    public DrDTime subtractInning(int inning) {
        return subtractInternal(inning, MAX_INNING, MULTIPLIER_INNING, this::subtractDay,
            this::getInning);
    }
    ...
    public DrDTime subtractCycle(int cycle) {
        return subtractInternal(cycle, MAX_CYCLE, MULTIPLIER_CYCLE, this::subtractInning,
            this::getCycle);
    }
