...
public final class InventoryContentDao extends BaseDatabaseService<InventoryRecord> {
    ...
    public synchronized CompletableFuture<Map<Integer, Integer>> getFreeSlotAsync(ItemBase item,
        int ammount) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            items.forEach(
                inventoryRecord -> occupiedSlots[inventoryRecord.getSlotId()] = SLOT_OCCUPIED);
            ...
            for (InventoryRecord inventoryRecord : items) {
                if (inventoryRecord.getItemId().equals(item.getId())) {
                    if (occupiedItemMap
                        .put(inventoryRecord.getSlotId(), stackSize - inventoryRecord.getAmmount())
                        != null) {
     ...
     */
    public synchronized Map<Integer, Integer> getFreeSlot(ItemBase item, int ammount)
